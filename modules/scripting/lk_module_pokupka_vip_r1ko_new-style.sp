...
{
	name = "[LK MODULE] Покупка VIP (VIP R1KO 3.0)",
...
public void LK_OnLoaded()
{
	...
	else
	{
		...
		LK_RegisterItem(g_sItemName, BuyVIPCallback);
	}
}
...
public void BuyVIPCallback(int iClient, int iItemID, const char[] sItemName)
{
...
public int MenuHandler_MainMenu2(Menu hMenu, MenuAction action, int iClient, int iItem)
{
	switch(action)
	{
		...
		case MenuAction_Select:
        {
			...
			{
				int SetVIPTime = g_Times[i][j][Days]*86400;
				VIP_GiveClientVIP(0, iClient, SetVIPTime, g_Groups[i][VIPGroup], true);
				LK_TakeClientCash(iClient, g_Times[i][j][Price]);
				if(g_Times[i][j][Days] == 0)
				{
