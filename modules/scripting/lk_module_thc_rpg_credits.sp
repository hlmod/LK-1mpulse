...
char g_sItemName[] = "thc_rpg_credits";

public Plugin myinfo =
{
	name = "[LK MODULE] Покупка RPG кредитов (THC RPG)",
...
{
	if(LK_GetVersion() < 400) LogError("[LK MODULE][Покупка RPG кредитов (THC RPG)] Обновите ядро до последней версии");
	else
	{
		LoadTranslations("lk_module_thc_rpg_credits.phrases");
...
public int MenuHandler_MainMenu(Menu hMenu, MenuAction action, int iClient, int iItem)
{
	switch(action)
	{
		...
		case MenuAction_Select:
        {
			...
			ServerCommand("thc_rpg_credits add @%s %i", userid, g_Credits[i][Credits]);
			LK_PrintToChat(iClient, "%T", "Buy_RPG_Credits", iClient, g_Credits[i][Credits]);
			LK_LogMessage("[Личный кабинет] Игрок %N (%s) купил %i RPG-кредитов", iClient, sAuth, g_Credits[i][Credits]);
			ShowMenuModule(iClient);
		}
	}
}
...
void KFG_load()
{
	...
	BuildPath(Path_SM, sPath, sizeof(sPath), "configs/lk/lk_module_thc_rpg_credits.ini");
	if(!KV.ImportFromFile(sPath)) SetFailState("[LK MODULE][Покупка RPG кредитов (THC RPG)] - Файл конфигураций не найден");
	KV.Rewind();
