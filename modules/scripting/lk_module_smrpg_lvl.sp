...
char g_sItemName[] = "smrpg_lvl";

public Plugin myinfo =
{
	name = "[LK MODULE] Покупка RPG уровней (SM:RPG)",
...
{
	if(LK_GetVersion() < 400) LogError("[LK MODULE][Покупка RPG уровней (SM:RPG)] Обновите ядро до последней версии");
	else
	{
		LoadTranslations("lk_module_smrpg_lvl.phrases");
...
public int MenuHandler_MainMenu(Menu hMenu, MenuAction action, int iClient, int iItem)
{
	switch(action)
	{
        ...
        {
			char szInfo[16], sAuth[32], userid[15];
			...
			ServerCommand("smrpg_addlvl #%s %i", userid, g_Credits[i][Credits]);
			LK_PrintToChat(iClient, "%T", "Buy_LvL_SMRPG", iClient, g_Credits[i][Credits]);
			LK_LogMessage("[Личный кабинет] Игрок %N (%s) купил %i RPG-уровней", iClient, sAuth, g_Credits[i][Credits]);
			ShowMenuModule(iClient);
		}
	}
}
...
void KFG_load()
{
	...
	BuildPath(Path_SM, sPath, sizeof(sPath), "configs/lk/lk_module_smrpg_lvl.ini");
	if(!KV.ImportFromFile(sPath)) SetFailState("[LK MODULE][Покупка RPG уровней (SM:RPG)] - Файл конфигураций не найден");
	KV.Rewind();
