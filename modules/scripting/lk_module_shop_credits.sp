...
char g_sItemName[] = "shop_credits";

public Plugin myinfo =
{
	name = "[LK MODULE] Покупка SHOP кредитов",
...
{
	if(LK_GetVersion() < 400) LogError("[LK MODULE][Покупка SHOP кредитов] Обновите ядро до последней версии");
	else
	{
		LoadTranslations("lk_module_shop_credits.phrases");
		LK_RegisterItem(g_sItemName, SHOPCred_Callback);
	}
}
...
public void SHOPCred_Callback(int iClient, int ItemID, const char[] ItemName)
{
...
public int MenuHandler_MainMenu(Menu hMenu, MenuAction action, int iClient, int iItem)
{
	switch(action)
	{
		...
		case MenuAction_Select:
        {
			...
			Shop_GiveClientCredits(iClient, g_Credits[i][Credits], IGNORE_FORWARD_HOOK);
			LK_PrintToChat(iClient, "%T", "Buy_Credits", iClient, g_Credits[i][Credits]);
			LK_LogMessage("[Личный кабинет] Игрок %N (%s) купил %i SHOP-кредитов", iClient, sAuth, g_Credits[i][Credits]);
			ShowMenuModule(iClient);
		}
	}
}
...
void KFG_load()
{
	...
	BuildPath(Path_SM, sPath, sizeof(sPath), "configs/lk/lk_module_shop_credits.ini");
	if(!KV.ImportFromFile(sPath)) SetFailState("[LK MODULE][Покупка SHOP кредитов] - Файл конфигураций не найден");
	KV.Rewind();
