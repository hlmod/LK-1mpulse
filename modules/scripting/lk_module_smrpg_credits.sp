...
char g_sItemName[] = "smrpg_credits";

public Plugin myinfo =
{
	name = "[LK MODULE] Покупка RPG кредитов (SM:RPG)",
...
{
	if(LK_GetVersion() < 400) LogError("[LK MODULE][Покупка RPG кредитов (SM:RPG)] Обновите ядро до последней версии");
	else
	{
		LoadTranslations("lk_module_smrpg_credits.phrases");
...
public int MenuHandler_MainMenu(Menu hMenu, MenuAction action, int iClient, int iItem)
{
	switch(action)
	{
		...
		case MenuAction_Select:
        {
			...
			ServerCommand("smrpg_addcredits #%s %i", userid, g_Credits[i][Credits]);
			LK_PrintToChat(iClient, "%T", "Buy_RPG_Credits", iClient, g_Credits[i][Credits]);
			LK_LogMessage("[Личный кабинет] Игрок %N (%s) купил %i кредитов в SM:RPG", iClient, sAuth, g_Credits[i][Credits]);
			ShowMenuModule(iClient);
		}
	}
}
...
void KFG_load()
{
	...
	BuildPath(Path_SM, sPath, sizeof(sPath), "configs/lk/lk_module_smrpg_credits.ini");
	if(!KV.ImportFromFile(sPath)) SetFailState("[LK MODULE][Покупка RPG кредитов (SM:RPG)] - Файл конфигураций не найден");
	KV.Rewind();
