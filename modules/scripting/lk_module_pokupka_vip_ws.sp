...
int g_iGroups, g_Groups[MAX_GROUPS][enum_Groups], g_Times[MAX_GROUPS][MAX_TIMES][enum_Times];
char g_sItemName[] = "pokupka_vip_ws";

public Plugin myinfo =
{
	name = "[LK MODULE] Покупка VIP (VIP wS)",
...
{
	if(LK_GetVersion() < 400) LogError("[LK MODULE][Покупка VIP (VIP wS)] Обновите ядро до последней версии");
	else
	{
		LoadTranslations("lk_module_pokupka_vip_ws.phrases");
...
public int MenuHandler_MainMenu2(Menu hMenu, MenuAction action, int iClient, int iItem)
{
	switch(action)
	{
		...
		case MenuAction_Select:
        {
			...
			if(VIP_GetSeconds(iClient) > -2)
			{
				if(VIP_GetSeconds(iClient) > 0)
				{
					char szBuffer[64];
					VIP_GetGroupName(iClient, szBuffer, sizeof(szBuffer));
					if(StrEqual(g_Groups[i][VIPGroup], szBuffer, true))
					{
						if(g_Times[i][j][Days] != 0)
						{
							...
							ServerCommand("sm_viptime + %i \"%s\"", g_Times[i][j][Days]*1440, szAuth);
			...
			else
			{
				...
				LK_TakeClientCash(iClient, g_Times[i][j][Price]);
				GetClientAuthId(iClient, AuthId_Steam2, szAuth, sizeof(szAuth), true);
				ServerCommand("sm_vipadd \"%s\" %i %s", szAuth, g_Times[i][j][Days]*1440, g_Groups[i][VIPGroup]);
				if(g_Times[i][j][Days] == 0)
				{
...
void KFG_load()
{
	...
	BuildPath(Path_SM, sPath, sizeof(sPath), "configs/lk/lk_module_pokupka_vip_ws.ini");
	if(!KV.ImportFromFile(sPath)) SetFailState("[LK MODULE][Покупка VIP (VIP wS)] - Файл конфигураций не найден");
	KV.Rewind();
