...
char g_sItemName[] = "wcs_private_race";

public Plugin myinfo =
{
	name = "[LK MODULE] Покупка WCS Private Race",
...
{
	if(LK_GetVersion() < 400) LogError("[LK MODULE][Покупка WCS Private Race] Обновите ядро до последней версии");
	else
	{
		LoadTranslations("lk_module_wcs_private_race.phrases");
...
public int MenuHandler_MainMenu(Menu hMenu, MenuAction action, int iClient, int iItem)
{
	switch(action)
	{
		...
		case MenuAction_Select:
        {
			...
			if(WCS_GivePrivateRace(sAuth, g_Credits[i][RaceName]))
			{
				LK_TakeClientCash(iClient, g_Credits[i][Price]);
				LK_PrintToChat(iClient, "%T", "Buy_WCSPrivateRace", iClient, g_Credits[i][RaceName]);
				LK_LogMessage("[Личный кабинет] Игрок %N (%s) купил приватную рассу %s", iClient, sAuth, g_Credits[i][RaceName]);
			}
			else LK_PrintToChat(iClient, "%T", "NoBuy_WCSPrivateRace", iClient);
			ShowMenuModule(iClient);
		}
	}
}
...
void KFG_load()
{
	...
	BuildPath(Path_SM, sPath, sizeof(sPath), "configs/lk/lk_module_wcs_private_race.ini");
	if(!KV.ImportFromFile(sPath)) SetFailState("[LK MODULE][Покупка WCS Private Race] - Файл конфигураций не найден");
	KV.Rewind();
	if(KV.GotoFirstSubKey(true))
	{
		...
		do 
		{
			if(KV.GetSectionName(g_Credits[g_iWcsGold][Name], 64))
			{
				...
				KV.GetString("race_name", g_Credits[g_iWcsGold][RaceName], 64);
