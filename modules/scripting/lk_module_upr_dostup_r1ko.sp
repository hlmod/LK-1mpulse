...
public void LK_OnLoaded()
{
	...
	else
	{
		...
		LK_RegisterItem(g_sItemName, UprVIPR1ko_Callback);
	}
}
...
public void UprVIPR1ko_Callback(int iClient, int ItemID, const char[] ItemName)
{
		...
		case MenuAction_Select:
		{
			switch(iItem)
			{
				case 0:
				{
					...
					VIP_GetClientVIPGroup(iClient, group, 65);
					VIP_RemoveClientVIP(iClient, true, true);
