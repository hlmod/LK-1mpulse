...
char g_sItemName[] = "shop_gold";

public Plugin myinfo =
{
	name = "[LK MODULE] Покупка SHOP Gold",
...
{
	if(LK_GetVersion() < 400) LogError("[LK MODULE][Покупка SHOP Gold] Обновите ядро до последней версии");
	else 
	{
		LoadTranslations("lk_module_shop_gold.phrases");
		LK_RegisterItem(g_sItemName, SHOPGold_Callback);
	}
}
...
public void SHOPGold_Callback(int iClient, int ItemID, const char[] ItemName)
{
...
public int MenuHandler_MainMenu(Menu hMenu, MenuAction action, int iClient, int iItem)
{
	switch(action)
	{
		...
		case MenuAction_Select:
        {
			...
			Shop_GiveClientGold(iClient, g_Credits[i][Credits], IGNORE_FORWARD_HOOK);
			LK_PrintToChat(iClient, "%T", "Buy_Gold", iClient, g_Credits[i][Credits]);
			LK_LogMessage("[Личный кабинет] Игрок %N (%s) купил %i Gold", iClient, sAuth, g_Credits[i][Credits]);
			ShowMenuModule(iClient);
		}
	}
}
...
void KFG_load()
{
	...
	BuildPath(Path_SM, sPath, sizeof(sPath), "configs/lk/lk_module_shop_gold.ini");
	if(!KV.ImportFromFile(sPath)) SetFailState("[LK MODULE][Покупка SHOP Gold] - Файл конфигураций не найден");
	KV.Rewind();
	if(KV.GotoFirstSubKey(true))
	{
		...
		do 
		{
			...
			{
				g_Credits[g_iShopItem][Credits] = KV.GetNum("gold");
