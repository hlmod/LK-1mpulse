...
char g_sItemName[] = "ncrpg_lvl";

public Plugin myinfo =
{
	name = "[LK MODULE] Покупка RPG уровней (NCRPG)",
...
{
	if(LK_GetVersion() < 400) LogError("[LK MODULE][Покупка RPG уровней (NCRPG)] Обновите ядро до последней версии");
	else
	{
		LoadTranslations("lk_module_ncrpg_lvl.phrases");
...
public int MenuHandler_MainMenu(Menu hMenu, MenuAction action, int iClient, int iItem)
{
	switch(action)
	{
		...
		case MenuAction_Select:
        {
			...
			NCRPG_SetLevel(iClient, NCRPG_GetLevel(iClient)+g_Credits[i][Credits]);
			LK_PrintToChat(iClient, "%T", "Buy_LvL_NCRPG", iClient, g_Credits[i][Credits]);
			LK_LogMessage("[Личный кабинет] Игрок %N (%s) купил %i уровней в NCRPG", iClient, sAuth, g_Credits[i][Credits]);
			ShowMenuModule(iClient);
		}
	}
}
...
void KFG_load()
{
	...
	BuildPath(Path_SM, sPath, sizeof(sPath), "configs/lk/lk_module_ncrpg_lvl.ini");
	if(!KV.ImportFromFile(sPath)) SetFailState("[LK MODULE][Покупка RPG уровней (NCRPG)] - Файл конфигураций не найден");
	KV.Rewind();
