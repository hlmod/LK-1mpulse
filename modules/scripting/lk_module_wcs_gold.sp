...
char g_sItemName[] = "wcs_gold";

public Plugin myinfo =
{
	name = "[LK MODULE] Покупка WCS Gold",
...
{
	if(LK_GetVersion() < 400) LogError("[LK MODULE][Покупка WCS Gold] Обновите ядро до последней версии");
	else
	{
		LoadTranslations("lk_module_wcs_gold.phrases");
...
public int MenuHandler_MainMenu(Menu hMenu, MenuAction action, int iClient, int iItem)
{
	switch(action)
	{
		...
		case MenuAction_Select:
        {
			...
			WCS_GiveGold(iClient, g_Credits[i][Credits]);
			LK_PrintToChat(iClient, "%T", "Buy_WCSGold", iClient, g_Credits[i][Credits]);
			LK_LogMessage("[Личный кабинет] Игрок %N (%s) купил %i WCS Gold", iClient, sAuth, g_Credits[i][Credits]);
			ShowMenuModule(iClient);
		}
	}
}
...
void KFG_load()
{
	...
	BuildPath(Path_SM, sPath, sizeof(sPath), "configs/lk/lk_module_wcs_gold.ini");
	if(!KV.ImportFromFile(sPath)) SetFailState("[LK MODULE][Покупка WCS Gold] - Файл конфигураций не найден");
	KV.Rewind();
	if(KV.GotoFirstSubKey(true))
	{
		...
		do 
		{
			...
			{
				g_Credits[g_iWcsGold][Credits] = KV.GetNum("gold");
